<template>
    <page-meta
        ...
        root-font-size="16px"
    >
        <navigation-bar front-color="#000000" background-color="#ff8440" />
    </page-meta>
    <view>
        <u-navbar title="立即提现" :autoBack="true" fixed class="w-ns"> </u-navbar>
        <view :style="{ height: '44px' }"> </view>
        <!-- <view class="ns">我的余额</view>:bgColor="'linear-gradient(to right, #ff8440, #ff1e0f)'"  -->
        <view class="flexView">
            <view class="scrollView">
                ...
                <view class="white-box">
                    <view class="tx">
                        ...
                        <view
                            class="tx-grid"
                            @click="navTo('/pages/earning/me/wallet/withdraw')"
                            ><span class="tx-grid-comm-sign"
                                >立即提现</span
                            ></view
                        >
                    </view>
                    <view class="palace palace-one">
                        <view class="palace-grid">
                            <view class="palace-grid-text">
                                <view class="palace-grid-text-data">
                                    <text>{{ user.waitIncome/100 }}</text>
                                    <text class="palace-grid-text-data-b"
                                        >元</text
                                    >
                                </view>
                                <view class="palace-grid-text-name"
                                    >即将到账</view
                                >
                            </view>
                        </view>
                        ...
                        <view class="palace-grid">
                            <view class="palace-grid-text">
                                <view class="palace-grid-text-data">
                                    <text>{{ user.historyWithdraw/100 }}</text>
                                    <text class="palace-grid-text-data-b"
                                        >元</text
                                    >
                                </view>
...
import { getWalletRevExp } from "@/api/index.js";
export default {
    components: {
        tuiTabs,
    },
    ...
    methods: {
        async loadData() {
            ...
            getWalletRevExp().then(r=>{
                this.allList=r.data
                this.list = this.allList['month']
            })
        },
        async getFundList() {
        ...
        changeTab(e) {
            ...
            if (this.currentTab == 3) {
                // this.getFundList()
                this.list = this.allList['withdrawal']
                ...
                this.list = this.allList['day']
                this.is_withdraw = false
            } else if (this.currentTab == 2) {
                this.list = this.allList['yesterday']
