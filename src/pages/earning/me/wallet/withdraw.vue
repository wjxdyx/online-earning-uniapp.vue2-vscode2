<template>
	...
	<view class="withdraw">
		<u-navbar title="提现" :autoBack="true" fixed > </u-navbar>
		...
		<view class="withdraw-body">
			...
			<view class="info-money">
				<view v-if="is_out">
					<text class="info-money-num" style="color: #ff1e0f;">输入金额超过可提现余额，账户余额{{  user.amt/100 }}元</text>
				</view>
				<view v-else-if="is_lowest">
					<text class="info-money-num" style="color: #ff1e0f;">最低{{ lowest }}元起提现，账户余额{{  user.amt/100 }}元</text>
				</view>
				<view v-else>
					<text class="info-money-num">当前可提现余额{{ user.amt/100 }}元，</text>
...
<script>
import { getWithdrawal } from "@/api/index.js";
export default {
	data() {
		return {
			pool: 0,
	...
	methods: {
		...
		async getMoney() {
			...
			if (this.isRefuse) return;
			this.isRefuse = true;
			await getWithdrawal({amt:this.money,mark:'申请到微信'}).then(r=>{
				console.log(r);
				this.$api.msg('提现申请已提交，等待管理员处理');
		
				return this.$store.dispatch('getMineInfo')
			}).then(r=>{
				console.log(r);
			}).catch(r=>{
