<template>
	<view class="HMfilterDropdown" :class="{'setDropdownBottom':maskVisibility}" :style="{'top':navMenuTop+'rpx'}" @touchmove.stop.prevent="discard" @tap.stop="discard">
		<!-- 顶部菜单 -->
...
<script>
	export default {
		...
		created(){
			...
			this.navMenuTop = Number(this.menuTop) + uni.getWindowInfo()['windowTop']
			console.log(	this.navMenuTop);
		},
		watch: {
			menuTop:function(newVal) {
				this.navMenuTop = Number(this.menuTop) + uni.getWindowInfo()['windowTop']
		...
		methods: {
			...
			//菜单开关
			togglePage(index,submenu) {
				if(this.isToggleing){return;}
				if(this.subData[index].type==='text'){return;}
			...
			confirm() {
				...
				index.forEach((item, i) => {
					if (typeof(item[0]) == 'object') {
						...
						if(!submenu){
							value[i][0] = this.subData[i].value;
						}else{
